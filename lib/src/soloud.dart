...
interface class SoLoud {
  ...
  // Kept as a method (not a setter) to preserve the existing fork API used
  // by app tests.
  @visibleForTesting
  ...
  Future<void> initSlave({
    ...
    // If the engine is already initialized in native, the developer may have
    // carried out a hot restart which does not imply the release of the
    // native player. Just deinit the engine to be re-inited below.
    if (nativeIsInitialized) {
      _log.warning(
